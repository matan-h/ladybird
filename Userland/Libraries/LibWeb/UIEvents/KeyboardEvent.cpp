...
static ErrorOr<String> get_event_key(KeyCode platform_key, u32 code_point, unsigned modifiers)
{
    ...
    // (at least in QT), alt and shift have the same as the normal, so only ctrl gets here
    else if (modifiers) {
    ...
    //               modifer keys removed except for glyph modifier keys.
        if ( code_point < 27) { // FIXME: corrently support only the alphabet due to issue #974 [ctrl+5 == ctrl+}].
            if (auto key_string = TRY(get_event_key_string(0x60 + code_point)); key_string.has_value()) {
                key = key_string.release_value();
            }
        }
    }
